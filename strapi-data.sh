...
do_export() {
    ...
    docker compose exec -T strapi-master yarn strapi export --exclude files --no-encrypt --file /tmp/database

    # Copier le fichier exporté depuis le container
    docker compose cp strapi-master:/tmp/database.tar.gz "$TEMP_DIR/database.tar.gz"
    log_success "Base de données exportée"

    # Nettoyer le fichier temporaire dans le container
    docker compose exec -T strapi-master rm -f /tmp/database.tar.gz

    # 2. Compression des uploads
    log_info "Compression des uploads..."

    # Vérifier si le dossier uploads existe dans le container
    if docker compose exec -T strapi-master test -d /app/bob/public/uploads; then
        docker compose exec -T strapi-master tar -czf /tmp/uploads.tar.gz -C /app/bob/public uploads
        docker compose cp strapi-master:/tmp/uploads.tar.gz "$TEMP_DIR/uploads.tar.gz"
        docker compose exec -T strapi-master rm -f /tmp/uploads.tar.gz
...
do_restore() {
    ...
    # 1. Copier l'archive dans le conteneur
    log_info "Copie de l'archive dans le conteneur..."
    docker compose cp "$ARCHIVE_FILE" strapi-master:/tmp/backup.tar.gz
    log_success "Archive copiée dans le conteneur"

    # 2. Décompresser l'archive dans le conteneur
    log_info "Décompression de l'archive dans le conteneur..."
    docker compose exec -T strapi-master mkdir -p /tmp/restore
    docker compose exec -T strapi-master tar -xzf /tmp/backup.tar.gz -C /tmp/restore
    log_success "Archive décompressée"

    # 3. Restaurer la base de données EN PREMIER
    # (l'import Strapi peut supprimer/recréer le dossier uploads)
    log_info "Restauration de la base de données..."

    if docker compose exec -T strapi-master test -f /tmp/restore/database.tar.gz; then
        docker compose exec -T strapi-master yarn strapi import --exclude files --file /tmp/restore/database.tar.gz --force
        log_success "Base de données restaurée"
    else
        log_error "Fichier database.tar.gz non trouvé dans l'archive"
        docker compose exec -T strapi-master rm -rf /tmp/restore /tmp/backup.tar.gz
        exit 1
    fi

    # 4. Restaurer les uploads APRÈS l'import de la base de données
    ...
    # 5. Nettoyer dans le conteneur
    log_info "Nettoyage dans le conteneur..."
    docker compose exec -T strapi-master rm -rf /tmp/restore /tmp/backup.tar.gz

    # Supprimer le fichier d'archive sur l'hôte
    ...
    log_warning "Redémarrez Strapi pour appliquer les changements: docker compose restart strapi-master"
}
